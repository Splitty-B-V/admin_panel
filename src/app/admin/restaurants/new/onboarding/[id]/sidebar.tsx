...
import { onboardingApi } from '../utils/onboarding-api'
...
const OnboardingSidebar: React.FC<OnboardingSidebarProps> = ({
    ...
    useEffect(() => {
            ...
            try {
                ...
                if (savedData) {
                    try {
                        ...
                        setProgressData({
                            completed_steps: parsedData.completedSteps || [],
                            personnel_added: parsedData.personnelData?.length || 0,
                            stripe_connected: parsedData.stripeData?.connected || false,
                            pos_configured: parsedData.posData?.posType ? true : false,
                            qr_stands_configured: parsedData.qrStandData?.isConfigured || false,
                            google_reviews_configured: parsedData.googleReviewData?.isConfigured || false,
                            telegram_configured: parsedData.telegramData?.isConfigured || false
                        })
                    } catch (e) {
        ...
        if (restaurant?.id && !restaurant.deleted) {
            loadProgress()
        } else {
            setLoadingProgress(false)
        }
        ...
        if (restaurant?.deleted) {
            ...
            if (savedData) {
                try {
                    ...
                    if (stepId === 6 && parsedData.telegramData) return 'available'
                } catch (e) {
    ...
    // Calculate actual progress from API data or completedSteps
    const actualCompletedSteps = progressData?.completed_steps || completedSteps
    const totalSteps = 6
    const progressPercentage = (actualCompletedSteps.length / totalSteps) * 100

    // Calculate personnel count from API or local state
    const personnelCount = progressData?.personnel_added || 0
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-600">{t('restaurants.onboarding.step')} {currentStep} {t('restaurants.onboarding.of')} {totalSteps}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="flex min-h-[calc(100vh-64px)] bg-gray-50">
                ...
                <div className="hidden lg:block w-80 bg-white border-gray-200 border-r fixed h-screen overflow-y-auto">
                    ...
                    <span className="text-xs font-medium text-gray-700">
                      ...
                      {actualCompletedSteps.length}/{totalSteps} completed
                    </span>
                                    </div>
                                    ...
                                    {/* Progress details from API */}
                                    {progressData && !loadingProgress && (
                                        <div className="mt-3 space-y-1">
                                            <div className="flex justify-between text-xs">
                                                <span className="text-gray-500">Personnel:</span>
                                                <span className={progressData.personnel_added > 0 ? 'text-[#2BE89A]' : 'text-gray-400'}>
                          {progressData.personnel_added > 0 ? `${progressData.personnel_added} added` : 'Not added'}
                        </span>
                                            </div>
                                            <div className="flex justify-between text-xs">
                                                <span className="text-gray-500">Stripe:</span>
                                                <span className={progressData.stripe_connected ? 'text-[#2BE89A]' : 'text-gray-400'}>
                          {progressData.stripe_connected ? 'Connected' : 'Not connected'}
                        </span>
                                            </div>
                                            <div className="flex justify-between text-xs">
                                                <span className="text-gray-500">POS:</span>
                                                <span className={progressData.pos_configured ? 'text-[#2BE89A]' : 'text-gray-400'}>
                          {progressData.pos_configured ? 'Configured' : 'Not configured'}
                        </span>
                                            </div>
                                        </div>
                                    )}
                            ...
                            <div className="space-y-2 pr-1">
                                ...
                                >
                                    <div className="p-4">
                                        ...
                                        <p className="text-xs text-gray-500 pl-7">Start your restaurant setup here</p>
                                    </div>
                                </button>
                ...
                {isMobileSidebarOpen && (
                    <div className="lg:hidden fixed inset-0 z-50 flex">
                      ...
                      {actualCompletedSteps.length}/{totalSteps}
                    </span>
                                    </div>
