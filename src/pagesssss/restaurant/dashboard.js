...
import DateFilterExact from '../components/DateFilterExact'
...
const generateTableData = () => {
  return [
    ...
    },
    {
      ...
      paid: 47.00,
      duration: '1u 20m',
      splitMode: 'equal', // Share bill equally
      startTime: '18:30',
      waiter: 'Lucas',
      section: 'Terras',
      createdAt: new Date(Date.now() - 80 * 60 * 1000),
      orderNumber: '20240012',
      orders: [
        { name: 'Steak Medium', price: 28.50 },
    ...
    },
    {
      ...
      paid: 0.00,
      duration: '2u 15m',
      // No split mode yet - nothing paid
      startTime: '18:00',
      waiter: 'Sophie',
      section: 'Privé zaal',
      ...
      orders: [
        { name: 'Mixed Grill (2x)', price: 67.00 },
        { name: 'Vegetarisch Menu (2x)', price: 48.00 },
      ...
      customPayments: [
        ...
        { guest: 'Gast 3', amount: 45.00, paid: false },
        { guest: 'Gast 4', amount: 35.00, paid: false },
        { guest: 'Gast 5', amount: 40.00, paid: false },
        { guest: 'Gast 6', amount: 44.43, paid: false }
      ]
    },
    {
      num: 8,
    ...
    },
    {
      ...
      orders: [
        { guest: 'Gast 1', items: [
          { name: 'Biefstuk', price: 26.50, paid: true },
          { name: 'Bier', price: 4.50, paid: true }
        ]},
        { guest: 'Gast 2', items: [
          { name: 'Spare Ribs', price: 22.50, paid: true },
          { name: 'Cola', price: 3.50, paid: false }
        ]},
        { guest: 'Gast 3', items: [
          { name: 'Zalm', price: 24.50, paid: false },
          { name: 'Witte Wijn', price: 6.50, paid: false }
        ]},
        { guest: 'Gast 4', items: [
          { name: 'Vegetarische Lasagne', price: 18.50, paid: false },
          { name: 'Spa Blauw', price: 3.00, paid: false }
        ]},
        { guest: 'Gast 5', items: [
    ...
    },
    {
      ...
      orderNumber: '20240009',
      orders: [
        { guest: 'Gast 1', items: [
          { name: 'Tournedos', price: 32.50, paid: true },
          { name: 'Rode Wijn', price: 7.50, paid: true }
        ]},
        { guest: 'Gast 2', items: [
          { name: 'Entrecote', price: 28.50, paid: true },
          { name: 'Bier', price: 4.50, paid: true }
        ]},
        { guest: 'Gast 3', items: [
          { name: 'Zeebaars', price: 26.50, paid: false },
          { name: 'Prosecco', price: 6.50, paid: false }
        ]},
        { guest: 'Gast 4', items: [
          { name: 'Risotto', price: 19.50, paid: false },
          { name: 'Water', price: 3.00, paid: false }
        ]},
        { guest: 'Gast 5', items: [
          { name: 'Hamburger', price: 18.50, paid: false },
          { name: 'Cola', price: 3.50, paid: false }
        ]},
        { guest: 'Gast 6', items: [
          { name: 'Pasta Pesto', price: 17.50, paid: false },
          { name: 'Ice Tea', price: 3.50, paid: false }
        ]},
        { guest: 'Gast 7', items: [
    ...
    },
    {
      ...
      customPayments: [
        ...
        { guest: 'Gast 3', amount: 30.40, paid: false }
      ]
    },
    {
      num: 18,
      ...
      orderNumber: '20240018',
      orders: [
        { name: 'Shared Platter XL', price: 85.00 },
    ...
    },
    {
      ...
      orderNumber: '20240011',
      orders: [
        { guest: 'Gast 1', items: [
          { name: 'Club Sandwich', price: 14.50, paid: true },
          { name: 'Cappuccino', price: 3.50, paid: true }
        ]},
        { guest: 'Gast 2', items: [
    ...
    },
    {
      ...
      orderNumber: '20240024',
      orders: [
        { name: 'Sushi Boat', price: 75.00 },
        { name: 'Tempura Mix', price: 35.00 },
        { name: 'Sake Fles', price: 42.50 },
...
const generateMockData = (filter, customDateRange = null) => {
  ...
  if (customDateRange && customDateRange.start && customDateRange.end) {
    ...
    for (let d = new Date(Math.max(startDate, launchDate)); d <= endDate; d.setDate(d.getDate() + 1)) {
...
const generateChartData = (filter, customDateRange = null) => {
  ...
  if (filter === 'today') {
    ...
    return [
      { time: `${t('time.weekDaysShort')[6]} 9`, omzet: 0, betalingen: 0, fooien: 0 }, // Saturday - CLOSED
      { time: `${t('time.weekDaysShort')[0]} 10`, omzet: 0, betalingen: 0, fooien: 0 }, // Sunday - CLOSED
      { time: `${t('time.weekDaysShort')[1]} 11`, omzet: 2423.50, betalingen: 44, fooien: 187.60 },
      { time: `${t('time.weekDaysShort')[2]} 12`, omzet: 2678.90, betalingen: 49, fooien: 198.70 },
      { time: `${t('time.weekDaysShort')[3]} 13`, omzet: 2891.20, betalingen: 53, fooien: 215.40 },
      { time: `${t('time.weekDaysShort')[4]} 14`, omzet: 3012.80, betalingen: 55, fooien: 232.10 },
      { time: `${t('time.weekDaysShort')[5]} 15`, omzet: 3287.60, betalingen: 61, fooien: 256.30 }
    ]
    ...
    const startDate = new Date(Math.max(new Date(customDateRange.start), launchDate))
    const endDate = new Date(customDateRange.end)
    const data = []
...
const generateRecentOrders = (filter) => {
  const baseOrders = [
  ...
  // Adjust based on filter (simplified for now)
  return baseOrders
}

// Stats Card Component
const StatsCard = ({ icon: Icon, title, value, change, subtitle, onClick, isClickable = false, isActive = false }) => (
  <div 
...
export default function Dashboard() {
  ...
  const [filteredData, setFilteredData] = useState(generateMockData('today'))
  ...
  const exportToPDF = () => {
    ...
    const generateTransactions = () => {
      ...
      const numTransactions = parseInt(filteredData.orders) || 50
    ...
    doc.setFontSize(14)
    doc.setFont(undefined, 'bold')
    doc.text(userData.restaurantName, pageWidth / 2, yPosition, { align: 'center' })
    yPosition += 8
        ...
        <div className="mb-8">
          ...
          {tables.filter(table => table.status === 'Bezet' || table.status === 'Wacht').length > 0 ? (
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
              ...
              const paymentProgress = table.status !== 'Vrij' ? (table.paid / table.amount) * 100 : 0
              return (
                <div 
                  key={table.num} 
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="bg-white rounded-lg p-2 border border-gray-100">
                          ...
                          <div className="flex justify-between items-baseline mt-0.5 xs:mt-1 pt-0.5 xs:pt-1 border-t border-gray-100">
                            ...
                            <span className="text-xs xs:text-sm font-bold text-orange-600">€{(table.amount - table.paid).toFixed(2)}</span>
                          </div>
                        </div>
                  ...
                  <tr key={payment.id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="px-2 sm:px-3 md:px-4 py-2 sm:py-2.5 whitespace-nowrap">
                      <span className={`inline-flex items-center px-1.5 sm:px-2 py-0.5 rounded-full text-xs font-medium ${
                        ...
                        <span className="hidden sm:inline">{payment.status === 'paid' ? t('dashboard.recentPayments.paid') : t('dashboard.recentPayments.failed')}</span>
                        <span className="sm:hidden">{payment.status === 'paid' ? '✓' : '✕'}</span>
                      </span>
                    </td>
                    <td className="px-2 sm:px-3 md:px-4 py-2 sm:py-2.5 whitespace-nowrap text-xs text-gray-500 hidden md:table-cell" suppressHydrationWarning>
                      {typeof window !== 'undefined' ? payment.time.toLocaleDateString('nl-NL', { 
                        month: 'short', 
                        day: 'numeric', 
                        year: 'numeric',
                        hour: '2-digit',
                    ...
                    <td className="px-2 sm:px-3 md:px-4 py-2 sm:py-2.5 whitespace-nowrap text-sm hidden lg:table-cell">
                      <div className="flex gap-1.5 items-center">
                        <button 
                          onClick={() => router.push(`/order/${payment.table}`)}
              ...
              <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
                <div className="flex items-center space-x-4">
                  ...
                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                    selectedTable.status === 'Bezet' ? 'bg-red-100 text-red-800' :
                    selectedTable.status === 'Wacht' ? 'bg-yellow-100 text-yellow-800' :
              ...
              <div className="p-6">
                ...
                <div className="grid grid-cols-2 gap-4 mb-6">
                  <div className="bg-gray-50 rounded-lg p-3">
                    ...
                    <p className="text-lg font-semibold text-gray-900">{selectedTable.duration}</p>
                    <p className="text-xs text-gray-400">{selectedTable.startTime}</p>
                  </div>
                  <div className="bg-gray-50 rounded-lg p-3">
                    <p className="text-xs text-gray-500 mb-1">Split Modus</p>
                    <div className="flex items-center space-x-1">
                      {(() => {
                        const Icon = getSplitModeIcon(selectedTable.splitMode)
                        return <Icon className="h-4 w-4 text-gray-600" />
                      })()}
                      <p className="text-sm font-semibold text-gray-900">{getSplitModeLabel(selectedTable.splitMode)}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-gradient-to-r from-gray-50 to-green-50 rounded-lg p-4 mb-6">
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-sm font-medium text-gray-700">Betaalvoortgang</span>
                    <span className="text-sm font-semibold text-gray-900">
                      €{selectedTable.paid.toFixed(2)} / €{selectedTable.amount.toFixed(2)}
                    </span>
                  </div>
                  <div className="w-full bg-white rounded-full h-3 overflow-hidden">
                    <div 
                      className="h-full bg-gradient-to-r from-green-500 to-emerald-500 rounded-full transition-all duration-300"
                      style={{ width: `${(selectedTable.paid / selectedTable.amount) * 100}%` }}
                    />
                  </div>
                  <p className="text-xs text-gray-500 mt-2">
                    {Math.round((selectedTable.paid / selectedTable.amount) * 100)}% betaald via Splitty
                  </p>
                </div>

                {/* Split Mode Specific Content */}
                {selectedTable.splitMode === 'items' && selectedTable.orders && (
                  <div className="space-y-4">
                    <h3 className="text-sm font-semibold text-gray-900 mb-3">Bestellingen per Gast</h3>
                    {selectedTable.orders.map((guestOrder, index) => (
                      <div key={index} className="border border-gray-200 rounded-lg p-4">
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center space-x-2">
                            <UserIcon className="h-4 w-4 text-gray-400" />
                            <span className="font-medium text-gray-900">{guestOrder.guest}</span>
                          </div>
                          <span className="text-sm text-gray-500">
                            €{guestOrder.items.reduce((sum, item) => sum + item.price, 0).toFixed(2)}
                          </span>
                        </div>
                        <div className="space-y-2">
                          {guestOrder.items.map((item, itemIndex) => (
                            <div key={itemIndex} className="flex items-center justify-between">
                              <div className="flex items-center space-x-2">
                                ...
                                <span className={`text-sm ${item.paid ? 'text-gray-500 line-through' : 'text-gray-700'}`}>
                                  {item.name}
                                </span>
                              </div>
                              <span className={`text-sm ${item.paid ? 'text-gray-400' : 'text-gray-900'}`}>
                    ...
                    {selectedTable.sharedItems && selectedTable.sharedItems.length > 0 && (
                      <div className="border border-gray-200 rounded-lg p-4 bg-gray-50">
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center space-x-2">
                            <UsersIcon className="h-4 w-4 text-gray-400" />
                            <span className="font-medium text-gray-900">Gedeelde Items</span>
                          </div>
                          <span className="text-sm text-gray-500">
                            €{selectedTable.sharedItems.reduce((sum, item) => sum + item.price, 0).toFixed(2)}
                          </span>
                        </div>
                        <div className="space-y-2">
                          {selectedTable.sharedItems.map((item, index) => (
                ...
                {selectedTable.splitMode === 'equal' && selectedTable.orders && (
                  <div className="space-y-4">
                    <h3 className="text-sm font-semibold text-gray-900 mb-3">Bestellingen</h3>
                      ...
                      {selectedTable.guests && (
                        <div className="border-t border-gray-200 mt-3 pt-3">
                          <div className="flex justify-between items-center">
                            <span className="text-sm font-medium text-gray-700">Per persoon ({selectedTable.guests} personen)</span>
                            <span className="text-sm font-bold text-gray-900">
                              €{(selectedTable.amount / selectedTable.guests).toFixed(2)}
                            </span>
                          </div>
                          {selectedTable.paidGuests !== undefined && (
                            <div className="mt-2">
                              <span className="text-xs text-gray-500">
                                {selectedTable.paidGuests} van {selectedTable.guests} personen hebben betaald
                              </span>
                            </div>
                          )}
                        </div>
                ...
                {selectedTable.splitMode === 'custom' && selectedTable.customPayments && (
                  <div className="space-y-4">
                    <h3 className="text-sm font-semibold text-gray-900 mb-3">Custom Betalingen</h3>
                    <div className="grid grid-cols-2 gap-3">
                      {selectedTable.customPayments.map((payment, index) => (
                        <div key={index} className="border border-gray-200 rounded-lg p-3">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-2">
                              ...
                              <span className="text-sm font-medium text-gray-700">{payment.guest}</span>
                            </div>
                            <span className={`text-sm font-semibold ${payment.paid ? 'text-green-600' : 'text-gray-900'}`}>
                    ...
                    {selectedTable.orders && (
                      <div className="border border-gray-200 rounded-lg p-4 bg-gray-50">
                        <h4 className="text-sm font-medium text-gray-700 mb-2">Bestelde Items</h4>
